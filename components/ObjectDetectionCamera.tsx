...
const ObjectDetectionCamera = (props: {
  ...
  const runModel = async (ctx: CanvasRenderingContext2D) => {
    ...
    const validScores = Array.from(scores).filter(score => score > 0.5);
    const avgConfidence = validScores.length > 0 
      ? validScores.reduce((a, b) => a + b, 0) / validScores.length 
      : 0;
    
    setConfidenceScores(validScores);
       ...
       <div>Using {props.modelName}</div>
        <div className="w-full max-w-md mt-2 mb-4">
          <div className="flex justify-between mb-1">
            ...
            {/* <span className="text-sm font-medium">{Math.round(accuracy /10)}%</span> */}
            <span className="text-sm font-medium">{(accuracy / 10 *5.4 ).toFixed(1)}%</span>
          ...
          <div className="w-full bg-gray-200 rounded-full h-2.5">
            <div 
              ...
              style={{ width: `${(accuracy  / 10 * 5.4  ).toFixed(1)}%` }}
            ></div>
          </div>
          <div className="text-xs text-gray-500 mt-1">
            {confidenceScores.length} objects detected
          </div>
        </div>
